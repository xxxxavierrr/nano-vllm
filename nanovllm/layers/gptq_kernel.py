...
def _gptq_w4a16_kernel(
    ...
    for k_start in range(0, K, BLOCK_K):
        ...
        packed_k = offs_k // 8
        weight_shift = (offs_k % 8) * 4
        packed_weight = tl.load(
            qweight_ptr + packed_k[:, None] * N + offs_n[None, :],
            mask=k_mask[:, None] & (offs_n[None, :] < N),
            other=0,
        )
        weight = (packed_weight >> weight_shift[:, None]) & 0xF

        group = tl.load(g_idx_ptr + offs_k, mask=k_mask, other=0)
        packed_n = offs_n // 8
...
def gptq_w4a16_linear(
    ...
    input_shape = x.shape
    output = _gptq_w4a16(x, qweight, scales, qzeros, g_idx)
    if bias is not None:
        output = output + bias
